...
```
### 5.cmd到根目录下，本地创建数据库的副本文件<br>
```bash
python manage.py makemigrations
```
### 6.副本文件推送到数据库，创造或修改表结构<br>
```bash
python manage.py migrate 
```
### 7.创建后台超级用户，用于登录后台管理<br>
```bash
python manage.py createsuperuser
```
### 8.安装VUE环境，下载node.js并配置环境，下载npm包管理器，安装yarn，安装 vue 脚手架用于生成 vue 工程模板<br>

```shell
yarn install --global vue-cli
```

### 9.cmd进入frontend目录下，安装相关依赖包<br>
...
![目录介绍](https://github.com/githublitao/api_automation_test/blob/master/img/目录介绍.jpeg)
